      ...
      <View className="flex-1 items-center justify-end px-4 pb-10">
        ...
        <View className="w-full max-w-sm rounded-[3rem] bg-white p-8 shadow-2xl">
          <View className="flex-row items-center justify-between mb-6">
            <Text className="text-lg font-black">Bills</Text>
          ...
          <ScrollView style={{ maxHeight: 380 }} showsVerticalScrollIndicator={false}>
            {loading ? (
              <View className="rounded-2xl bg-slate-50 p-4">
                <Text className="text-sm text-slate-400">Loading bills...</Text>
              </View>
            ) : error ? (
              ...
              <View className="rounded-2xl bg-slate-50 p-4">
                <Text className="text-sm text-slate-400">No bills yet.</Text>
              </View>
            ) : (
              <View>
                {sortedBills.map((bill) => (
                  <Pressable
                    ...
                    className={`w-full rounded-2xl p-3 mb-3 ${
                      editingId === bill.id ? 'bg-white border border-black/80' : 'bg-slate-50'
                    }`}
                  >
                    <View className="flex-row items-center justify-between">
                      <Text className="font-semibold text-slate-900">{bill.name}</Text>
                      <Text className="font-semibold text-slate-900">{formatAmount(bill.amount, currencyCode)}</Text>
                    </View>
                    <View className="mt-1 flex-row items-center justify-between">
                      <Text className="text-xs text-slate-400">{bill.cadence}</Text>
                      <Text className="text-xs text-slate-400">{formatDueDate(bill.nextDueDate)}</Text>
                    </View>
                  </Pressable>
                ))}
              </View>
