...
const useFinanceData = ({ user, refreshToken = 0 }: UseFinanceDataParams) => {
    ...
    const mapped = entries
      .map((entry) => {
        ...
        return {
          ...
          iconName: getTransactionIconName(category, type),
  ...
  const safeAccounts = hasUser ? accounts : [];
  const safeGoals = hasUser ? goals : [];
  const safeTransactions = hasUser ? transactions : [];
  const safeRawEntries = hasUser ? rawEntries : [];
  const safeBills = hasUser ? bills : [];
  const safeSubscriptions = hasUser ? subscriptions : [];
  const safeAccountsLoading = hasUser ? accountsLoading : false;
  const safeGoalsLoading = hasUser ? goalsLoading : false;
  const safeEntriesLoading = hasUser ? entriesLoading : false;
  const safeBillsLoading = hasUser ? billsLoading : false;
  const safeSubscriptionsLoading = hasUser ? subscriptionsLoading : false;
  const safeAccountsError = hasUser ? accountsError : null;
  const safeGoalsError = hasUser ? goalsError : null;
  const safeEntriesError = hasUser ? entriesError : null;
  const safeBillsError = hasUser ? billsError : null;
  const safeSubscriptionsError = hasUser ? subscriptionsError : null;

  return {
    accounts: safeAccounts,
    goals: safeGoals,
    transactions: safeTransactions,
    rawEntries: safeRawEntries,
    bills: safeBills,
    subscriptions: safeSubscriptions,
    setTransactions,
    setRawEntries,
    accountsLoading: safeAccountsLoading,
    goalsLoading: safeGoalsLoading,
    entriesLoading: safeEntriesLoading,
    billsLoading: safeBillsLoading,
    subscriptionsLoading: safeSubscriptionsLoading,
    accountsError: safeAccountsError,
    goalsError: safeGoalsError,
    entriesError: safeEntriesError,
    billsError: safeBillsError,
    subscriptionsError: safeSubscriptionsError,
    fetchAccounts,
    fetchGoals,
    fetchEntries,
    fetchBills,
